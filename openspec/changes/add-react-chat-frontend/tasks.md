...
## 7. 流式模式优化
- [x] 7.1 实现混合流式模式（`stream_mode=["messages", "updates"]`）
- [x] 7.2 区分处理 token 级消息和节点级更新
- [x] 7.3 修复重复 assistant 消息问题

## 8. UI 增强功能
- [x] 8.1 实现 localStorage 持久化（线程列表、活跃线程）
- [x] 8.2 添加主题切换功能（暗黑/浅色模式，绿色滑块开关）
- [x] 8.3 优化输入框交互（移除 Send 按钮，Enter 发送）
- [x] 8.4 统一 UI 颜色方案，优化视觉一致性
- [x] 8.5 更新品牌为 "GravAIty"，优化相关 UI 元素
- [x] 8.6 添加聊天模型选择器（qwen-plus-latest, qwen-max-latest, qwen-flash）
- [x] 8.7 实现线程删除功能（三个点菜单 + 删除按钮）
- [x] 8.8 添加复制 thread ID 功能（复制到剪贴板 + toast 提示）
- [x] 8.9 实现动态标题生成（从第一条用户消息提取前30字符）
- [x] 8.10 添加 AI 回复操作按钮（复制、点赞、点踩）
- [x] 8.11 修复用户消息对齐（消息头和时间戳右对齐）
- [x] 8.12 优化 New Chat 按钮样式（黑色背景、四角星图标）
